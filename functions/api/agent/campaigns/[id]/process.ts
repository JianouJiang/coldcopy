// POST /api/agent/campaigns/:id/process
// Triggers cron processing for a specific campaign's pending tasks.
// Called by the frontend after campaign creation or via "Process Now" button.
...
// Plan limits (duplicated from cron.ts to keep this self-contained)
...
export async function onRequestPost(context: any): Promise<Response> {
  ...
  // Pick up pending tasks for this campaign (up to 5)
  const { results: tasks } = await env.DB.prepare(
    "SELECT * FROM agent_tasks WHERE campaign_id = ? AND status = 'pending' AND attempts < 3 ORDER BY created_at ASC LIMIT 5"
  ).bind(campaignId).all();

  if (!tasks || tasks.length === 0) {
    return new Response(JSON.stringify({ message: 'No pending tasks', processed: 0, results: [] }), {
      headers: { 'content-type': 'application/json' },
    });
  }
  ...
  for (const task of tasks as any[]) {
    try {
      ...
      ).bind(newStatus, err.message, task.id).run();

      results.push({ id: task.id, type: task.task_type, status: 'failed', error: err.message });
    }
  }

  // Check if there are MORE pending tasks after this batch (pipeline creates new tasks)
  const { results: remaining } = await env.DB.prepare(
    "SELECT COUNT(*) as count FROM agent_tasks WHERE campaign_id = ? AND status = 'pending' AND attempts < 3"
  ).bind(campaignId).all();
  const hasMore = remaining && (remaining[0] as any)?.count > 0;

  return new Response(JSON.stringify({ processed: results.length, results, has_more: hasMore }), {
    headers: { 'content-type': 'application/json' },
  });
}

// --- Task Processors (same logic as cron.ts) ---
...
Respond with one line per result: just KEEP or SKIP, in order.`,
  ...
  for (const result of filteredResults) {
    ...
    ).bind(task.campaign_id, result.link).first();
    if (existing) continue;

    const leadId = generateId();
    await env.DB.prepare(
      'INSERT INTO leads (id, campaign_id, company_name, company_url, company_description, source, status) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).bind(leadId, task.campaign_id, result.title, result.link, result.snippet, 'web_search', 'new').run();

    const findTaskId = generateId();
    await env.DB.prepare(
      'INSERT INTO agent_tasks (id, campaign_id, task_type, payload) VALUES (?, ?, ?, ?)'
    ).bind(findTaskId, task.campaign_id, 'find_emails', JSON.stringify({ lead_id: leadId })).run();
  ...
  ).bind(leadsCreated, task.campaign_id).run();
}
...
NAME: NONE`,
  ...
  ).bind(JSON.stringify(researchData), contactEmail, contactName, newStatus, lead_id).run();
  ...
  if (contactEmail) {
    ...
    await env.DB.prepare(
      'INSERT INTO agent_tasks (id, campaign_id, task_type, payload) VALUES (?, ?, ?, ?)'
    ).bind(genTaskId, task.campaign_id, 'generate_emails', JSON.stringify({ lead_id })).run();
  }
}
...
async function processGenerateEmails(task: any, env: any) {
  ...
  const campaign: any = await env.DB.prepare('SELECT * FROM campaigns WHERE id = ?').bind(task.campaign_id).first();
  if (!campaign) throw new Error('Campaign not found');

  // Check plan-based daily draft limit (same as send limit — no point generating more drafts than you can send)
  const userRow: any = await env.DB.prepare('SELECT plan, name FROM users WHERE id = ?').bind(campaign.user_id).first();
  ...
  if (draftsToday && draftsToday.count >= planLimits.max_emails_per_day) {
    throw new Error(`LIMIT_REACHED: Daily email limit reached (${planLimits.max_emails_per_day}/day on ${userRow?.plan || 'free'} plan). Upgrade for more.`);
  }

  const researchData = JSON.parse(lead.research_data || '{}');
  const senderName = campaign.sender_name || userRow?.name || 'the team';
...
Generate a personalized cold email in English. Sign off with the sender's real name (${senderName}).`;
  ...
  if (!subjectMatch || bodyMatch.length < 2) throw new Error('Could not parse Claude response format');

  const emailData = { subject: subjectMatch[1].trim(), body: bodyMatch[1].trim() };
  if (!emailData.subject || !emailData.body) throw new Error('Missing subject or body in response');

  const emailId = generateId();
  await env.DB.prepare(
    'INSERT INTO outbound_emails (id, lead_id, campaign_id, subject, body, status) VALUES (?, ?, ?, ?, ?, ?)'
  ).bind(emailId, lead_id, task.campaign_id, emailData.subject, emailData.body, 'draft').run();

  await env.DB.prepare("UPDATE leads SET status = 'email_generated' WHERE id = ?").bind(lead_id).run();
}
...
async function processSendEmails(task: any, env: any) {
  ...
  const lead: any = await env.DB.prepare('SELECT * FROM leads WHERE id = ?').bind(email.lead_id).first();
  if (!lead || !lead.contact_email) throw new Error('Lead email not found');

  const campaign: any = await env.DB.prepare('SELECT * FROM campaigns WHERE id = ?').bind(email.campaign_id).first();
  if (!campaign) throw new Error('Campaign not found');
  ...
  const { messageId, threadId } = await sendEmail(
    accessToken, gmailConn.gmail_email, lead.contact_email, email.subject, email.body
  );

  await env.DB.prepare(
    "UPDATE outbound_emails SET status = 'sent', gmail_message_id = ?, gmail_thread_id = ?, sent_at = datetime('now') WHERE id = ?"
  ).bind(messageId, threadId, email.id).run();

  await env.DB.prepare("UPDATE leads SET status = 'sent' WHERE id = ?").bind(email.lead_id).run();

  await env.DB.prepare(
    "UPDATE campaigns SET emails_sent = emails_sent + 1, updated_at = datetime('now') WHERE id = ?"
  ).bind(email.campaign_id).run();

  const checkTaskId = generateId();
  await env.DB.prepare(
    'INSERT INTO agent_tasks (id, campaign_id, task_type, payload) VALUES (?, ?, ?, ?)'
  ).bind(checkTaskId, email.campaign_id, 'check_replies', JSON.stringify({ campaign_id: email.campaign_id })).run();
}
...
async function processCheckReplies(task: any, env: any) {
  ...
  const campaign: any = await env.DB.prepare('SELECT user_id FROM campaigns WHERE id = ?').bind(campaignId).first();
  if (!campaign) throw new Error('Campaign not found');
  ...
  for (const email of sentEmails as any[]) {
    try {
      ...
      if (existingReplies && existingReplies.count >= replyLimits.max_reply_rounds) continue;
